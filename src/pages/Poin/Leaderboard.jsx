...
class Leaderboard extends Component {
    ...
    componentDidMount = () => {
        ...
        this.props.getLeaderboardPengguna({...this.state.routeParams, limit:20}).then((result)=>{
            this.setState({
                ...this.state,
                leaderboard_pengguna: result.payload
            })
        })
                ...
                <Row style={{marginBottom:'50px'}}>
                    ...
                    <Col width="100" tabletWidth="80" desktopWidth="70">
                        <Block style={{marginTop:'8px', marginBottom:'8px'}}>
                            <Segmented raised>
                                <Button className="color-theme-deeporange" style={{color:'#ffffff', fontWeight:'bold'}} tabLink="#tab-1" tabLinkActive>Koneksi Anda</Button>
                                <Button className="color-theme-deeporange" style={{color:'#434343', fontWeight:'bold'}} tabLink="#tab-2" onClick={()=>this.$f7router.navigate('/LeaderboardGlobal/')}>Global</Button>
                            </Segmented>
                        </Block>
                        <Card>
                            <CardContent style={{padding:'8px'}}>
                                <BlockTitle style={{marginTop:'0px'}}>Koneksi Anda</BlockTitle>
                                {this.state.leaderboard_pengguna.rows.map((option)=>{
                                    return (
                                            ...
                                            <CardContent style={{padding:'8px'}}>
                                                ...
                                                <Row>
                                                    <Col width="10" tabletWidth="10" desktopWidth="5" style={{fontSize:'15px', fontWeight:'bold', paddingTop:'14px'}}>
                                                        #{(this.state.leaderboard_pengguna.rows.indexOf(option)+1)}
                                                    </Col>
                                                    ...
                                                    <Col width="25" tabletWidth="15" desktopWidth="15" style={{textAlign:'right', display:(parseInt(this.state.leaderboard_pengguna.rows.indexOf(option)) === 0 ? 'inline-table' : 'block')}}>
                                                        <b style={{fontSize:'20px', color:'#558b2f'}}>{this.formatAngka(option.poin)}</b>
                                                        {parseInt(this.state.leaderboard_pengguna.rows.indexOf(option)) === 0 &&
