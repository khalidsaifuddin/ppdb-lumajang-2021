...
class kehadiranRekapSiswa extends Component {
    state = {
        error: null,
        loading: true,
        loadingKuis: true,
    ...
    loadData = () => {
        ...
        this.setState({
            ...
            this.props.kehadiranRekapSiswa(this.state.routeParams).then((result)=>{

                let sumPersen = 0;
                let totalPersen = 0;
                let totalSiswa = 0;
                
                console.log(this.props.kehadiran_rekap_siswa)

                this.props.kehadiran_rekap_siswa.map((option)=>{
                    ...
                    if(parseInt(this.state.pengaturan_sekolah['masuk_0'+option.urut_hari]) === 1){
                        ...
                        sumPersen = sumPersen + (option.persen ? (parseInt(option.total)/parseInt(option.total_siswa)*100) : 0);
                        
                        if(option.sekolah_id){
                            totalPersen++;
                        }                        

                        if(option.total_siswa){
                            totalSiswa = option.total_siswa;
                        }
    ...
    unduhLaporan = (bulan) => {
        // alert(bulan)
        window.open(localStorage.getItem('api_base')+"/api/Sekolah/unduhLaporanKehadiranSiswa?sekolah_id="+this.$f7route.params['sekolah_id']+"&bulan="+bulan+"&tahun="+this.state.tahun)
    }

    render()
    {
        return (
            <Page name="kehadiranRekapSiswa" hideBarsOnScroll>
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    <NavTitle sliding>Rekap Kehadiran Harian Siswa</NavTitle>
                </Navbar>
                <Card>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <Gauge
                                                    style={{padding:'8px'}}
                                                    type="circle"
                                                    value={(optionHari.total ? ((optionHari.total)/parseFloat(optionHari.total_siswa)) : 0)}
                                                    size={250}
                                                    borderColor="#2196f3"
                                                    // borderWidth={10}
                                                    valueText={`${parseFloat((optionHari.total ? (parseFloat(optionHari.total)/parseFloat(optionHari.total_siswa)) : 0) * 100).toFixed(0)}%`}
                                                    valueFontSize={80}
                                                ...
                                                <div style={{fontSize:'10px'}}>
                                                    {optionHari.total} dari {optionHari.total_siswa} siswa
                                                </div>
                                                }
                                                {!optionHari.total &&
                                                <div style={{fontSize:'10px'}}>
                                                ...
                                                //     style={{padding:'4px'}}
                                                //     type="circle"
                                                //     value={(optionHari.total ? (parseFloat(optionHari.total)/parseFloat(optionHari.total_siswa)) : '0')}
                                                //     size={250}
                                                //     borderColor="#2196f3"
                                                //     // borderWidth={10}
                                                //     valueText={`${(optionHari.total ? (parseFloat(optionHari.total)/parseFloat(optionHari.total_siswa)) : 0) * 100}%`}
                                                //     valueFontSize={80}
                                                //     valueTextColor="#2196f3"
                                                //     // labelText="Guau"
                                                //     labelFontWeight={800}
                            ...
                            <Col width="50">
                                <Card className={this.state.loading ? "skeleton-text skeleton-effect-blink label-cell" : ""}>
                                    ...
                                    <CardContent style={{minHeight:'65px'}}>
                                        ...
                                        dari total {this.state.total_siswa} siswa   
                                    </CardContent>
                                </Card>
                            </Col>
                            <Col width="50">
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      kehadiranRekapSiswa: actions.kehadiranRekapSiswa,
...
function mapStateToProps({ App, Sekolah, Guru }) {
    return {
        ...
        kehadiran_rekap_siswa: Guru.kehadiran_rekap_siswa,
        pengaturan_sekolah: Sekolah.pengaturan_sekolah
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(kehadiranRekapSiswa));
