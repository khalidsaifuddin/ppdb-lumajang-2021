...
class vervalGuru extends Component {
    state = {
        error: null,
        loading: true,
        loadingKuis: true,
    ...
    verifikasi = () => {
        ...
        if(parseInt(this.state.sekolah_pengguna_record.valid) === 3){
            this.$f7.dialog.confirm('Apabila guru ini ditolak, maka yang bersangkutan akan langsung dihapus dari daftar guru di sekolah Anda. Apakah Anda yakin ingin menolak guru ini?', 'Peringatan', ()=>{
                // alert('ditolak!');
                this.setState({
                    loading: true,
                    sekolah_pengguna_record: {
                    ...
                    this.props.simpanSekolahPengguna(this.state.sekolah_pengguna_record).then((result)=>{
                        if(result.payload.sukses){
                            //berhasil
                            this.$f7router.navigate('/daftarGuru/'+this.state.sekolah_pengguna_record.sekolah_id);
    ...
    {
        return (
            <Page name="vervalGuru" hideBarsOnScroll>
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    <NavTitle sliding>Verifikasi Guru</NavTitle>
                </Navbar>
...
export default (connect(mapStateToProps, mapDispatchToProps)(vervalGuru));
