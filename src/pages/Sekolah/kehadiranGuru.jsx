...
class kehadiranGuru extends Component {
    state = {
        error: null,
        loading: true,
        loadingKuis: true,
    ...
    componentDidMount = () => {
        ...
        this.props.getKehadiranGuru(this.state.routeParams).then((result)=>{
            console.log(this.props.kehadiran_guru); 
            this.$f7.dialog.close();
            if(this.props.kehadiran_guru.total > 0){

                this.setState({
                    kehadiran_guru_record: this.props.kehadiran_guru.rows[0],
                    kehadiran_guru_total: this.props.kehadiran_guru.total
                });
    ...
    catatKehadiran = (tipe) => {
        ...
        if(tipe === 'hadir'){
            ...
            this.setState({
                kehadiran_guru_record: {
                    ...this.state.kehadiran_guru_record,
                ...
                console.log(this.state.kehadiran_guru_record);
                this.props.simpanKehadiranGuru(this.state.kehadiran_guru_record).then((result)=>{
                    if(result.payload.sukses){
                        //berhasil!
                        this.$f7.dialog.close();
                        this.$f7.dialog.alert('Selamat bekerja!', 'Berhasil menyimpan kehadiran!');
                        this.props.getKehadiranGuru(this.state.routeParams).then((result)=>{
                            this.setState({
                                kehadiran_guru_record: this.props.kehadiran_guru.rows[0],
                                kehadiran_guru_total: this.props.kehadiran_guru.total
    ...
    {
        return (
            <Page name="kehadiranGuru" hideBarsOnScroll>
                <Navbar sliding={false} backLink="Kembali" onBackClick={this.backClick}>
                    <NavTitle sliding>Kehadiran Guru</NavTitle>
                </Navbar>
                <Row>
                        ...
                        <div style={{textAlign:'center', width:'100%'}}>
                            <Card>
                                <CardContent>
                                    ...
                                    {this.state.kehadiran_guru_record.waktu_datang &&
                                    <div style={{fontSize:'15px', color: 'teal', fontWeight:'normal', marginTop:'16px'}}>
                                        Waktu Hadir: {moment(this.state.kehadiran_guru_record.waktu_datang).format('H') + ':' + moment(this.state.kehadiran_guru_record.waktu_datang).format('mm') + ':' + moment(this.state.kehadiran_guru_record.waktu_datang).format('ss')}
                                    </div>
                                    }
                                    {this.state.kehadiran_guru_record.waktu_pulang &&
                                    <div style={{fontSize:'15px', color: 'teal', fontWeight:'normal', marginTop:'0px'}}>
                                        Waktu Pulang: {moment(this.state.kehadiran_guru_record.waktu_pulang).format('H') + ':' + moment(this.state.kehadiran_guru_record.waktu_pulang).format('mm') + ':' + moment(this.state.kehadiran_guru_record.waktu_pulang).format('ss')}
                                    </div>
                                    }
                                    {this.state.pengaturan_sekolah &&
                                    <Button disabled={((this.state.pengaturan_sekolah && parseInt(this.state.pengaturan_sekolah.radius_absen_aktif) !== 1 ? (!this.state.kehadiran_guru_record.waktu_pulang ? false : true) : (parseFloat(this.props.jarak_sekolah.m) <= parseFloat(this.state.pengaturan_sekolah.radius_absen_sekolah_guru) ? (!this.state.kehadiran_guru_record.waktu_pulang ? false : true) : true) ) )} raised fill large className={(this.state.kehadiran_guru_total > 0 ? "button-absen color-theme-teal bawahCiriHijau" : "button-absen color-theme-red bawahCiri")} onClick={()=>this.catatKehadiran((this.state.kehadiran_guru_total > 0 ? 'pulang' : 'hadir'))}>
                                        <img src={localStorage.getItem('api_base')+"/assets/img/125503_white.png"} style={{height:'70px'}} />
                                        <br/>
                                        <div>{(this.state.kehadiran_guru_total > 0 ? (!this.state.kehadiran_guru_record.waktu_pulang ? 'Pulang' : '-') : 'Hadir')}</div>
                                    </Button>
                                    }
                                    {!this.state.pengaturan_sekolah &&
                                    <Button disabled={(!this.state.kehadiran_guru_record.waktu_pulang ? false : true)} raised fill large className={(this.state.kehadiran_guru_total > 0 ? "button-absen color-theme-teal bawahCiriHijau" : "button-absen color-theme-red bawahCiri")} onClick={()=>this.catatKehadiran((this.state.kehadiran_guru_total > 0 ? 'pulang' : 'hadir'))}>
                                        <img src={localStorage.getItem('api_base')+"/assets/img/125503_white.png"} style={{height:'70px'}} />
                                        <br/>
                                        <div>{(this.state.kehadiran_guru_total > 0 ? (!this.state.kehadiran_guru_record.waktu_pulang ? 'Pulang' : '-') : 'Hadir')}</div>
                                    </Button>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      ...
      getKehadiranGuru: actions.getKehadiranGuru,
      simpanKehadiranGuru: actions.simpanKehadiranGuru,
...
function mapStateToProps({ App, Sekolah, Guru }) {
    return {
        ...
        kehadiran_guru: Guru.kehadiran_guru,
        jarak_sekolah: Sekolah.jarak_sekolah,
        pengaturan_sekolah: Sekolah.pengaturan_sekolah
    }
}

export default (connect(mapStateToProps, mapDispatchToProps)(kehadiranGuru));
